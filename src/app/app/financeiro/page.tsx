...
import { 
  Building2, RefreshCw, Plus, ArrowLeft, TrendingUp, 
  Wallet, ExternalLink, Loader2 
} from "lucide-react";
...
export default function FinanceiroPage() {
  ...
  useEffect(() => {
    // Só busca se o usuário já tiver uma empresa vinculada
    if (!localUser?.empresa_id) return;

    const fetchConexoes = async () => {
      try {
        setIsLoading(true);
        // Busca as conexões bancárias EXCLUSIVAS da empresa logada
        ...
        // Se não tiver dados reais ainda, injetamos um "mock" para manter o visual bonito 
        // até você plugar a API bancária real.
    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-4xl font-black italic uppercase tracking-tighter text-gray-900 leading-none">Financeiro</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-[35px] p-8 text-white shadow-xl relative overflow-hidden group">
          ...
          <div className="relative z-10">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60 italic mb-2">Saldo Consolidado</p>
        ...
        <div className="bg-white border border-gray-100 rounded-[35px] p-8 shadow-sm hover:shadow-lg transition-all">
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 italic mb-2">Previsão Saídas</p>
          <h2 className="text-4xl font-black italic uppercase tracking-tighter text-red-500">R$ 12.800,00</h2>
          <p className="mt-6 text-gray-400 font-bold text-xs uppercase italic tracking-widest">Próximos 7 dias</p>
        </div>
        ...
        <div className="bg-white border border-gray-100 rounded-[35px] p-8 shadow-sm border-l-4 border-l-orange-500 hover:shadow-lg transition-all flex flex-col justify-between">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 italic mb-2">DDA Pendente</p>
            <h2 className="text-4xl font-black italic uppercase tracking-tighter text-gray-900">08 Boletos</h2>
          </div>
          <Link href="/app/financeiro/dda" className="mt-6 inline-flex items-center gap-2 text-orange-600 font-black uppercase italic text-[10px] hover:text-orange-700 tracking-widest">
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-black italic uppercase tracking-tighter text-gray-800">Cofres Conectados</h2>
          <Button variant="outline" className="rounded-[15px] border-orange-200 text-orange-600 font-black italic uppercase text-[10px] tracking-widest px-6 h-12 hover:bg-orange-50">
        ...
        <div className="grid gap-4">
          {isLoading ? (
            <div className="py-10 flex justify-center"><Loader2 className="animate-spin text-orange-500 w-10 h-10" /></div>
          ) : conexoes.map((c, idx) => (
            <Card key={c.id || idx} className="border-gray-100 rounded-[30px] hover:border-orange-500 hover:shadow-xl transition-all bg-white group cursor-pointer">
              <CardContent className="p-6 flex items-center justify-between">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h3 className="text-xl font-black italic uppercase text-gray-800 tracking-tighter">{c.banco_nome}</h3>
                ...
                <Button variant="ghost" className="text-gray-400 group-hover:text-orange-600 font-black italic uppercase text-[10px] tracking-widest">
                  Ver Transações
                </Button>
              </CardContent>
            </Card>
