    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          <Link href="/app" className="p-3 bg-white border border-gray-100 rounded-2xl hover:bg-purple-50 text-gray-400 hover:text-purple-600 transition-all shadow-sm">
            <ArrowLeft size={20} />
          </Link>
          <div>
            ...
            <p className="text-gray-400 font-black italic uppercase text-[10px] tracking-[0.2em]">Inteligência aplicada à redução de custos</p>
          </div>
        </div>
      ...
      {/* Interface de Chat / Consultoria IA */}
      <div className="bg-white rounded-[45px] border border-gray-100 shadow-2xl overflow-hidden flex flex-col min-h-[550px] relative">
        <div className="absolute inset-0 bg-gradient-to-b from-purple-50/30 to-transparent pointer-events-none"></div>
        ...
        <div className="p-8 border-b border-gray-50 bg-white/50 backdrop-blur-md flex items-center justify-between relative z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-black italic text-gray-900 uppercase tracking-tighter text-lg">Consultor Estratégico</h3>
              <div className="flex items-center gap-2">
                 <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                 <p className="text-[10px] text-emerald-600 font-black uppercase tracking-widest italic">IA Online • Pronto para analisar</p>
              </div>
            </div>
          </div>
          <div className="hidden md:block">
             <span className="text-[9px] font-black uppercase tracking-widest text-gray-400 bg-gray-50 px-3 py-1.5 rounded-full border border-gray-100">Cérebro: GPT-4o Premium</span>
          </div>
        </div>
        ...
        <div className="p-10 bg-white relative z-10">
          <div className="relative max-w-4xl mx-auto group">
            <div className="absolute -inset-1 bg-gradient-to-r from-purple-600 to-blue-600 rounded-[30px] blur opacity-20 group-hover:opacity-40 transition duration-1000"></div>
            <input 
              type="text" 
...
function InsightCard({ title, desc, icon, action, color }: any) {
  return (
    <div className={`p-8 rounded-[40px] ${color} border border-white shadow-sm flex flex-col h-full justify-between hover:shadow-xl hover:-translate-y-1 transition-all duration-500 group`}>
      <div>
        ...
        <h3 className="font-black italic text-gray-900 uppercase text-md tracking-tighter mb-2">{title}</h3>
        <p className="text-gray-500 text-xs font-bold italic leading-relaxed mb-6 opacity-80">{desc}</p>
      </div>
...
function SuggestionButton({ text }: { text: string }) {
  return (
    <button className="text-left p-5 rounded-[22px] border border-gray-100 bg-white hover:border-purple-200 hover:bg-purple-50 transition-all text-xs font-black italic uppercase tracking-tighter text-gray-500 flex items-center justify-between group">
      "{text}" <ArrowRight size={16} className="text-purple-600 opacity-0 group-hover:opacity-100 transition-all -translate-x-2 group-hover:translate-x-0" />
    </button>
  );
}
