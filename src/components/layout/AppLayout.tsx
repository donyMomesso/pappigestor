      ...
      <header className="bg-white/80 backdrop-blur-xl border-b border-gray-100 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-24 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <Link
              href="/app"
              className="p-2 bg-gradient-to-br from-orange-500 to-pink-500 rounded-2xl shadow-lg shadow-orange-200"
            >
              <img
                src={LOGO_URL}
                alt="Logo"
            ...
            <div>
              ...
              <span className="text-[8px] font-black uppercase tracking-[0.3em] text-gray-400 italic">
                SaaS Inteligente
              </span>
            </div>
          </div>
          ...
          <nav className="hidden lg:flex items-center gap-1 bg-gray-50 p-1.5 rounded-[22px] border border-gray-100">
            ...
            <NavItem
              href="/app/compras"
              active={currentPath.startsWith("/app/compras")}
              icon={<ShoppingCart size={18} />}
              label="Compras"
            />
            <NavItem
              href="/app/financeiro"
              active={currentPath.startsWith("/app/financeiro")}
              icon={<DollarSign size={18} />}
              label="Financeiro"
            />
            <NavItem
              href="/app/estoque"
              active={currentPath.startsWith("/app/estoque")}
          ...
          <div className="flex items-center gap-5">
            <div className="hidden md:block text-right">
              <p className="text-xs font-black italic uppercase tracking-tighter text-gray-900">
                {localUser?.nome}
              </p>
              <div className="flex justify-end items-center gap-1">
                <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></div>
                <p className="text-[9px] uppercase tracking-widest text-orange-600 font-black italic">
                  Acesso {localUser?.nivel_acesso}
                </p>
              </div>
            </div>
            ...
            <div className="group relative">
              ...
              <div className="absolute right-0 mt-4 w-60 bg-white rounded-[30px] shadow-2xl border border-gray-50 opacity-0 group-hover:opacity-100 transition-all pointer-events-none group-hover:pointer-events-auto p-4 translate-y-2 group-hover:translate-y-0 z-50">
                <p className="px-2 text-[9px] font-black uppercase text-gray-400 italic tracking-widest mb-3">
                  Opções da Unidade
                </p>
                <Link
                  href="/app/configuracoes"
                  className="block px-4 py-3 text-[10px] font-black uppercase italic text-gray-600 hover:bg-orange-50 rounded-xl transition-all"
                >
                  Ajustes da Empresa
                </Link>
                <button
                  onClick={() => signOut()}
                  className="w-full flex items-center gap-3 px-4 py-3 mt-1 text-[10px] font-black uppercase italic text-red-500 hover:bg-red-50 rounded-xl transition-all"
                >
  ...
  return (
    <Link
      ...
      className={`flex items-center gap-2 px-6 py-3 rounded-2xl transition-all duration-300 ${
        active
          ? "bg-white text-orange-600 font-black italic shadow-xl shadow-orange-100/50 border border-gray-100 scale-105"
