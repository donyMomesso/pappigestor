...
interface EmpresaConfigResponse {
  empresa: EmpresaData;
  config: EmpresaConfigData;
}
...
function ConfiguracoesContent() {
  ...
  const [config, setConfig] = useState<EmpresaConfigData | null>(null);
  ...
  async function bootstrap() {
    try {
      ...
      const sessionRes = await fetch("/api/app/session", {
        cache: "no-store",
      });

      const session = (await sessionRes.json()) as SessionResponse & { error?: string };

      if (!sessionRes.ok) {
        throw new Error(session?.error || "Falha ao carregar sessão");
      }

      const currentEmpresaId = session?.empresaAtual?.id || null;

      if (!currentEmpresaId) {
        throw new Error("Empresa atual não encontrada na sessão");
      }

      setEmpresaId(currentEmpresaId);

      await Promise.all([
        fetchConfig(currentEmpresaId),
  ...
  async function fetchConfig(currentEmpresaId: string) {
    ...
    const data = (await res.json()) as EmpresaConfigResponse & { error?: string };

    if (!res.ok) {
      throw new Error(data?.error || "Erro ao carregar configuração da empresa");
    }
    ...
    setFormData({
      limite_aprovacao_pagamento:
        data.config?.limite_aprovacao_pagamento?.toString() || "",
      limite_aprovacao_compra:
        data.config?.limite_aprovacao_compra?.toString() || "",
      whatsapp_admin: data.config?.whatsapp_admin || "",
      openai_api_key: data.config?.openai_api_key || "",
      gemini_api_key: data.config?.gemini_api_key || "",
      provedor_ia: data.config?.provedor_ia || "gemini",
      modelo_ia: data.config?.modelo_ia || "gemini-1.5-flash",
    });
  }
  ...
  async function handleSaveConfig() {
    if (!empresaId) {
      alert("Empresa atual não identificada");
      return;
    }
        ...
        <CardContent>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="rounded-lg bg-gray-50 p-3">
              ...
              <p className="text-sm font-medium capitalize">{empresa?.plano || "-"}</p>
            </div>
            <div className="rounded-lg bg-gray-50 p-3">
              <Label className="text-xs text-gray-500">Status</Label>
              <p className="text-sm font-medium capitalize">{empresa?.status || "-"}</p>
            </div>
            <div className="rounded-lg bg-gray-50 p-3">
              <Label className="text-xs text-gray-500">CNPJ</Label>
              <p className="font-mono text-sm font-medium">{empresa?.cnpj || "-"}</p>
            </div>
          </div>
        </CardContent>
      </Card>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-gray-800">
                  {assinaturaStatus?.empresa_nome || empresa?.name || "Minha Empresa"}
                </p>
