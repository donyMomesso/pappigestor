...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    try {
      // Chama sua rota de login que inicia o OAuth
      const res = await fetch("/api/auth/login", { method: "POST" });
      if (!res.ok) throw new Error("Falha ao iniciar login");
      const { url } = await res.json();
      window.location.href = url; // redireciona para Google OAuth
    } catch (err) {
      ...
      <div className="w-full md:w-[750px] p-8 md:p-16 flex flex-col justify-center bg-white overflow-y-auto">
        <div className="w-full max-w-xl mx-auto space-y-12">
          ...
          <div className="pt-8 border-t border-gray-100 flex items-start gap-4">
            ...
            <div>
              <p className="text-xs font-black italic uppercase text-gray-800 tracking-tight">
                Ambiente Seguro & Isolado
              </p>
...
function PricingCard({ plan, price, features, color, highlight, icon }: any) {
  return (
    <div
      className={`p-6 rounded-[35px] border flex flex-col justify-between transition-all hover:shadow-2xl hover:-translate-y-1 ${color} cursor-default`}
    >
      <div>
        ...
        <p className="text-2xl font-black italic tracking-tighter mb-4 leading-none">
          {price}
          <span className="text-[10px] opacity-60 ml-1">/mês</span>
        </p>
      </div>
      <ul className="space-y-2">
        {features.map((f: string) => (
          <li
            key={f}
            className="text-[8px] font-black uppercase italic flex items-center gap-2 leading-none"
          >
