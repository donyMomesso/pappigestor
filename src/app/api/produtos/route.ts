...
import { getSupabaseAdmin, normalizeId, resolveEmpresaId } from "@/lib/pappi-server";
...
function normalizeProduto(row: any) {
  return {
    ...
    fornecedor_preferencial_id:
      row.fornecedor_preferencial_id ?? row.fornecedor_padrao_id ?? null,
  };
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!error) {
      return NextResponse.json((data || []).map(normalizeProduto));
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!nome) {
      return NextResponse.json(
        { error: "nome_produto obrigatório" },
        { status: 400 }
      );
    }
    ...
    const payload: Record<string, any> = {
      ...
      nome_produto: nome,
      categoria: String(
        body?.categoria_produto || body?.categoria || "Outros"
      ),
      ...
      unidade_medida: String(
        body?.unidade_medida || body?.unidade || "un"
      ),
      fornecedor_preferencial_id: body?.fornecedor_preferencial_id
        ? normalizeId(body.fornecedor_preferencial_id)
        : null,
      codigo_barras: body?.codigo_barras
        ? String(body.codigo_barras)
        : null,
      marca: body?.marca ? String(body.marca) : null,
      descricao: body?.descricao ? String(body.descricao) : null,
      peso_embalagem: body?.peso_embalagem
        ? String(body.peso_embalagem)
        : null,
      preco_referencia:
        body?.preco_referencia != null
          ? Number(body.preco_referencia)
          : null,
      catalogo_base_id: body?.catalogo_base_id
        ? String(body.catalogo_base_id)
        : null,
    };
    ...
    const produtoExistente = db.produtos.find(
      (produto) =>
        produto.nome_produto.toLowerCase() === nome.toLowerCase()
    );

    if (produtoExistente) {
      return NextResponse.json(
        { error: "Produto já existe", produto: produtoExistente },
        { status: 409 }
      );
    }
    ...
    const produto = createProduto(db, {
      ...
      fornecedor_preferencial_id: body?.fornecedor_preferencial_id
        ? String(body.fornecedor_preferencial_id)
        : null,
      codigo_barras: body?.codigo_barras
        ? String(body.codigo_barras)
        : null,
