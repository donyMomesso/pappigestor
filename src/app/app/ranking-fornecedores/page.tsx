...
import { Trophy, Star, Loader2, ArrowLeft, Filter } from "lucide-react";
  ...
  useEffect(() => {
    const fetchRanking = async () => {
      try {
        ...
        if (res.ok) {
          const data = (await res.json()) as RankingFornecedor[];
          setRanking(Array.isArray(data) ? data : []);
        } else {
          setRanking([]);
        }
      } catch (error) {
        console.error("Erro ao carregar ranking de fornecedores:", error);
        setRanking([]);
      } finally {
        setIsLoading(false);
      }
      ...
      <div className="grid gap-6">
        {isLoading ? (
          ...
          ranking.map((f, idx) => (
            <Card
              key={f.id}
              className="border-gray-100 rounded-[35px] shadow-sm hover:shadow-xl transition-all group overflow-hidden bg-white"
            >
              <CardContent className="p-0">
                <div className="flex items-center p-8 gap-8">
                  <div className="relative">
                    <div
                      className={`w-16 h-16 rounded-[22px] flex items-center justify-center text-2xl font-black italic ${
                        idx === 0
                  ...
                  <div className="flex-1">
                    <h3 className="text-xl font-black italic uppercase text-gray-800 tracking-tight">
                      {f.fornecedor_nome}
                    </h3>
                  ...
                  <div className="grid grid-cols-2 gap-8 text-right">
                    <div>
                      ...
                      <p className="text-lg font-black italic text-green-600">
                        R${" "}
                        {(f.economia_total ?? 0).toLocaleString("pt-BR")}
                      </p>
                    </div>
                    <div>
                      <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">
