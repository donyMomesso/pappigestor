...
export default function LoginPage() {
  ...
  const handleMagicLink = async (e: React.FormEvent) => {
    ...
    if (!supabase) {
      setMessage("❌ Supabase não configurado (ENV ausente).");
      setLoading(false);
      return;
    }
    ...
    if (error) {
      if (error.message.toLowerCase().includes("rate limit")) {
        setMessage(
          "❌ Limite de e-mails atingido. Por favor, use 'Entrar com Google' ou aguarde 1 hora."
        );
      } else {
  ...
  // 2) Login com Google (COM GMAIL SCOPES)
  const loginGoogle = async () => {
    ...
    // se não houver erro, o Supabase redireciona sozinho
  };

  const LOGO_URL = "/logo.png";

  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6 bg-[url('/banner.png')] bg-cover bg-center relative">
      ...
      <div className="max-w-4xl w-full grid grid-cols-1 md:grid-cols-2 bg-white rounded-[40px] shadow-2xl overflow-hidden border border-white relative z-10">
        ...
        <div className="p-12 flex flex-col justify-center bg-white">
          ...
          {message && (
            <div
              className={`mt-6 p-4 rounded-2xl text-[10px] font-black text-center italic uppercase tracking-tighter ${
                message.includes("✅")
