...
export default function UsuariosPage() {
  ...
  return (
    <div className="space-y-8">
      {/* Header do Módulo de Utilizadores */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium italic text-sm">Controle de acessos, permissões e gestão de equipa</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                <th className="px-6 py-4 text-[10px] uppercase tracking-widest font-black text-gray-400">Usuário</th>
                <th className="px-6 py-4 text-[10px] uppercase tracking-widest font-black text-gray-400">Nível</th>
                <th className="px-6 py-4 text-[10px] uppercase tracking-widest font-black text-gray-400">Status</th>
                <th className="px-6 py-4 text-[10px] uppercase tracking-widest font-black text-gray-400">Último Acesso</th>
                <th className="px-6 py-4 text-[10px] uppercase tracking-widest font-black text-gray-400 text-right">Ações</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50 font-medium">
              {/* Linha do Utilizador Atual (Exemplo) */}
              <tr className="hover:bg-gray-50/50 transition-colors">
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-gray-800 font-bold">{localUser?.nome || "Usuário Master"}</p>
                      <p className="text-xs text-gray-400 lowercase">{localUser?.email || "admin@saas.com"}</p>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-1.5 text-emerald-600">
                    ...
                    <span className="text-xs font-bold uppercase tracking-tighter">Ativo</span>
                  </div>
                </td>
...
function UserStatCard({ title, value, sub, icon, color }: any) {
  return (
    <div className={`bg-white p-6 rounded-4xl border-2 ${color} shadow-sm hover:shadow-md transition-all`}>
      ...
      <h3 className="text-gray-500 font-black text-xs uppercase tracking-widest leading-none mb-1">{title}</h3>
      <p className="text-3xl font-black italic text-gray-800 tracking-tighter">{value}</p>
      <p className="text-[11px] font-bold text-gray-400 mt-1 uppercase">{sub}</p>
    </div>
  );
}