    ...
    const fetchProducts = async () => {
      try {
        ...
        if (localUser?.empresa_id) {
          headers["x-empresa-id"] = String(localUser.empresa_id);
        }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest italic">
              Gestão de insumos e mercadorias
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
        {isLoading ? (
          ...
          filtered.map((p) => (
            <Card
              key={p.id}
              className="border-gray-100 rounded-[35px] shadow-sm hover:shadow-2xl transition-all group overflow-hidden flex flex-col"
            >
              <div className="h-2 bg-gradient-to-r from-orange-500 to-pink-500 opacity-20 group-hover:opacity-100 transition-opacity" />
              <CardContent className="p-7 flex flex-col flex-1">
                <div className="flex justify-between items-start mb-6">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-lg font-black italic text-orange-600">
                      R${" "}
                      {typeof p.ultimo_preco_pago === "number"
                        ? p.ultimo_preco_pago.toFixed(2)
                        : "0,00"}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  <span className="text-[9px] px-2 py-1 rounded-lg bg-gray-50 text-gray-500 font-black uppercase italic border border-gray-100">
                    ...
                    {p.categoria_produto || "Sem categoria"}
                  </span>
                ...
                <div className="mt-auto pt-6 border-t border-gray-50 flex justify-between items-center">
                  ...
                  <div className="flex gap-1">
                    <Button
                      variant="ghost"
                      size="icon"
                      className="rounded-xl hover:bg-orange-50 text-gray-300 hover:text-orange-500"
                    >
                      <Edit2 size={16} />
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="rounded-xl hover:bg-red-50 text-gray-300 hover:text-red-500"
                    >
                      <Trash2 size={16} />
                    </Button>
                  </div>
