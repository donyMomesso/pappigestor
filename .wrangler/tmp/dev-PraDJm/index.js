...
app.use("/api/*", async (c, next) => {
  ...
  if (!pizzariaId) {
    return c.json({ error: "Acesso negado: Unidade n\xE3o identificada." }, 401);
  }
...
app.post("/api/empresas/minhas", async (c) => {
  try {
    ...
    if (!email) return c.json({ error: "Email \xE9 obrigat\xF3rio." }, 400);
...
app.post("/api/auth/login", async (c) => {
  try {
    ...
    const user = await c.env.DB.prepare(`
        ...
        WHERE u.email = ? AND u.status = 'ativo'
      `).bind(email).first();
    if (!user || user.p_status !== "ativo") return c.json({ error: "Acesso negado." }, 403);
    return c.json({ pizzaria_id: user.pizzaria_id, role: user.funcao, nome: user.nome });
  } catch (e) {
...
app.get("/api/admin/usuarios", async (c) => {
  ...
  try {
    ...
    return c.json({
      usuarios: results ?? [],
      plano: { nome: "Gr\xE1tis", limite, usado }
    });
  } catch (e) {
    return c.json(
      { error: "Erro ao listar usu\xE1rios", details: e?.message ?? String(e) },
      500
    );
  }
});
app.post("/api/admin/usuarios", async (c) => {
  ...
  try {
    ...
    return c.json(
      { error: "Erro ao associar usu\xE1rio", details: e?.message ?? String(e) },
      500
    );
  }
});
app.delete("/api/admin/usuarios/:id", async (c) => {
  ...
  if (!id) return c.json({ error: "ID inv\xE1lido." }, 400);
  try {
    const row = await c.env.DB.prepare(
      "SELECT funcao FROM perfis_usuarios WHERE id = ? AND empresa_id = ? LIMIT 1"
    ).bind(id, empresaId).first();
    if (!row) return c.json({ error: "Usu\xE1rio n\xE3o encontrado." }, 404);
    if (row.funcao === "master") {
      return c.json({ error: "N\xE3o \xE9 permitido remover o usu\xE1rio master." }, 403);
    }
    await c.env.DB.prepare("DELETE FROM perfis_usuarios WHERE id = ? AND empresa_id = ?").bind(id, empresaId).run();
    return c.json({ success: true });
  } catch (e) {
    return c.json(
      { error: "Erro ao remover usu\xE1rio", details: e?.message ?? String(e) },
      500
    );
  }
});
app.post("/api/ia/ler-nota", async (c) => {
  ...
  try {
    ...
    const base64Image = Buffer2.from(arrayBuffer).toString("base64");
...
app.post("/api/ia/ler-link", async (c) => {
  try {
    ...
    const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
    const prompt = `
        Abaixo est\xE1 o c\xF3digo HTML de uma Nota Fiscal (NFC-e) do Brasil.
        ...
        C\xF3digo HTML: ${htmlContent.substring(0, 35e3)} 
      `;
...
app.post("/api/ia/interpretar-lista", async (c) => {
  try {
    ...
    return c.json({ error: "N\xE3o foi poss\xEDvel compreender o texto." }, 500);
  }
});
app.get("/api/ia/inbox-count", async (c) => {
  const pId = c.get("pizzariaId");
  try {
    const { total } = await c.env.DB.prepare("SELECT COUNT(*) as total FROM caixa_entrada WHERE pizzaria_id = ? AND status = 'pendente'").bind(pId).first() || { total: 0 };
...
app.post("/api/ia/inbox/approve", async (c) => {
  ...
  try {
    ...
    return c.json({ success: true, message: "Lan\xE7ado!" });
  } catch (e) {
    return c.json({ error: "Falha na aprova\xE7\xE3o" }, 500);
  }
});
...
app.delete("/api/fornecedores/:id", async (c) => {
  ...
  app.post("/api/fornecedores", async (c2) => {
    const pId2 = c2.get("pizzariaId");
    try {
      ...
      return c2.json({ error: "Erro ao criar fornecedor." }, 500);
    }
  });
});
...
app.post("/api/lista-compras", async (c) => {
  ...
  try {
    ...
    await c.env.DB.prepare("INSERT INTO lista_compras (pizzaria_id, produto_id, quantidade_solicitada) VALUES (?, ?, ?)").bind(pId, produto_id, quantidade_solicitada).run();
    return c.json({ success: true });
  } catch (e) {
...
app.get("/api/admin/usuarios", async (c) => {
  const pId = c.get("pizzariaId");
  try {
    const pizzaria = await c.env.DB.prepare("SELECT plano, limite_usuarios FROM pizzarias WHERE id = ?").bind(pId).first();
    if (!pizzaria) return c.json({ error: "Unidade n\xE3o encontrada" }, 404);
    ...
    return c.json({ error: "Erro ao buscar equipe" }, 500);
  }
});
app.post("/api/admin/usuarios", async (c) => {
  const pId = c.get("pizzariaId");
  ...
  try {
    const count = await c.env.DB.prepare("SELECT COUNT(*) as total FROM perfis_usuarios WHERE pizzaria_id = ?").bind(pId).first();
    const pizzaria = await c.env.DB.prepare("SELECT limite_usuarios FROM pizzarias WHERE id = ?").bind(pId).first();
    ...
    return c.json({ error: "E-mail j\xE1 associado." }, 400);
  }
});
...
// .wrangler/tmp/bundle-W9uJiC/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-W9uJiC/middleware-loader.entry.ts
